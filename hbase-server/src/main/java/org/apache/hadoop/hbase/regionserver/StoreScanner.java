...
public class StoreScanner extends NonReversedNonLazyKeyValueScanner
  ...
  public StoreScanner(HStore store, ScanInfo scanInfo, Scan scan, NavigableSet<byte[]> columns,
    ...
    try {
      ...
      // 根据 TimeRange, KeyRange过滤掉不需要的HFile
      // 根据 BloomFilter 过滤HFile
      // scanner包含 memstorescanner 和 storefilescanner
      ...
      // family marker.
      // seek是针对KV的， seek到指定KV，如果不存在则seek到指定KV的下一个
  ...
  public boolean next(List<Cell> outResult, ScannerContext scannerContext) throws IOException {
    ...
    LOOP: do {
      ...
      topChanged = false;
      // 返回下一步的seek操作 INCLUDE_AND_SEEK_NEXT_ROW / INCLUDE_AND_SEEK_NEXT_COL...
