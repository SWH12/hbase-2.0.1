...
public class StoreFileScanner implements KeyValueScanner {
  ...
  public boolean requestSeek(Cell kv, boolean forward, boolean useBloom)
    ...
    if (haveToSeek) {
      ...
      // kv的ts 与 hfile中最大的ts相比
      // 当kv的ts比HFile中最大的ts都更大时，那么这个HFile中显然不存在seekKV，
      // 但是可能存在相同rowkey,family,qualifier的不同ts的KeyValue,
      // 那么这里设置堆顶时要注意，
      // 不能把堆顶设置为比当前HFile文件中的可能真实存在的相同rowkey,family,qualifier的KeyValue大
